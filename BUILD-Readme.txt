...
git clone https://github.com/dzimbeck/MorningStar.git
cd MorningStar
chmod +x linuxMorningStar.sh
./linuxMorningStar.sh --build

Linux x86_64:

git clone https://github.com/dzimbeck/MorningStar.git
cd MorningStar
chmod +x linux64MorningStar.sh
./linux64MorningStar.sh --build

Options:
--reset: update dependencies, clone MorningStar repository
--force: if you are using other versions such as Debian version > 7

Example with options:
./linux64MorningStar.sh --reset --force --build

After successful build, morningstard can be found in MorningStar/src/
