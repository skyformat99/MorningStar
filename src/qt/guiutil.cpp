...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("morningstar"))
        return false;
...
{
    // Convert morningstar:// to morningstar:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("morningstar://"))
    {
        uri.replace(0, 12, "morningstar:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "MorningStar.lnk";
}
...
{
    return GetAutostartDir() / "morningstar.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("MorningStar-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  morningstar-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("MorningStar-Qt"));
...
void SetMorningStarThemeQSS(QApplication& app)
{
...
void SetMorningStarFonts(QWidget * w) {
#ifdef Q_OS_MAC
    QFont font("Roboto", 15);
#else
    QFont font("Roboto", 11);
